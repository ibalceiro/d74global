...
<table <?php if ($classes): print 'class="' . $classes . '" '; endif; ?><?php print $attributes; ?>>
  ...
  <tbody>
    <?php foreach ($rows as $row_count => $row): 
      $bg = '#BCF5A9';
      $remain = explode(" ", $row['field_license_expiration_date'])[6];
      if(empty($remain) || $remain <= 30){ $bg = "#F6CECE"; }
      elseif ($remain > 30 && $remain <= 60){ $bg = "#F3E2A9"; } 
    ?>
      <tr style="background-color: <?php echo $bg; ?>" <?php if ($row_classes[$row_count]): print 'class="' . implode(' ', $row_classes[$row_count]) . '"'; endif; ?>>
